...
async function structureWithClaude(rawNews, date, useSearch = false) {
  ...
  const sys = `Senior PE/RE investment analyst. Rules: Korean headlines max 55 chars. Categories: 글로벌|일본|미국|아시아|매크로|딜|화제|한국. ALWAYS include 딜 (M&A, PE, RE transactions, activist, earnings with angle) and 화제 (sports, AI/tech, geopolitics, trending). implications = specific PE/RE action point. Real URLs only. NO Japanese punctuation in Korean text (use Korean . and , only).`;
